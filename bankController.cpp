...
BankController::BankController(int ratingColumn, QObject *parent)
    : QSqlRelationalDelegate(parent), ratingColumn(ratingColumn)
{}

void BankController::paint(QPainter *painter,
                         ...
                         const QModelIndex &index) const
{
    if (index.column() != ratingColumn) {
        ...
        // Does Icon for book rating, Can use for our assets later.
...
// Does sizing
QSize BankController::sizeHint(const QStyleOptionViewItem &option,
                             const QModelIndex &index) const
{
    if (index.column() == ratingColumn)
...
// Event handler
bool BankController::editorEvent(QEvent *event, QAbstractItemModel *model,
                               const QStyleOptionViewItem &option,
...
QWidget *BankController::createEditor(QWidget *parent,
                                    const QStyleOptionViewItem &option,
                                    const QModelIndex &index) const
{
